...
export const GET: APIRoute = async ({ params, request }) => {
  ...
  try {
    ...
    if (format === 'svg') {
      ...
      const pngErr = svgToPng(svgError);
  ...
  const svg = makeSvg({ title: esc(title), subtitle: esc(subtitle), types: types.map(esc), address: esc(formatted_address) , id });
  if (format === 'svg') {
    return new Response(svg, { headers: headers('svg') });
  }
  const plain = makePlainSvg({ title: esc(title), subtitle: esc(subtitle), types: types.map(esc), address: esc(formatted_address), id });
...
function makeSvg({ title, subtitle, types, address, id }: SvgData) {
  const gradientId = 'g';
  const typeBadges = types
    .map((t) => `<span class=\"badge\">${t}</span>`)
    .join('');
  // Inline CSS (limited subset supported by most crawlers rendering SVG -> image)
  const css = `
  ...
  // We cannot easily inline the QR without foreignObject parsing issues for raster; skip QR in rich SVG for now.
...
function makePlainSvg({ title, subtitle, types, address, id }: SvgData) {
  ...
  const qrRenderSize = 140; // pixels within OG image
  ...
  for (let r = 0; r < moduleCount; r++) {
    for (let c = 0; c < moduleCount; c++) {
      if (qr.isDark(r, c)) {
        const x = c * cellSize;
        const y = r * cellSize;
        qrSvgParts += `<rect x="${x.toFixed(2)}" y="${y.toFixed(2)}" width="${cellSize.toFixed(2)}" height="${cellSize.toFixed(2)}" fill="#0f172a"/>`;
      }
    }
  }
